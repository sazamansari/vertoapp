...
export const TaskViewSwitcher = ({ projectId, hideProjectFilter }: TaskViewSwitcherProps) => {
  ...
  const onKanbanChange = useCallback(
    (tasks: { $id: string; status: TaskStatus; position: number }[]) => {
      bulkUpdateTasks({
        json: { tasks, workspaceId },
      });
    },
    [bulkUpdateTasks, workspaceId],
  );
        ...
        {isLoadingTasks ? (
          ...
          <>
            <TabsContent value="table" className="mt-0">
              <DataTable columns={columns as any} data={tasks?.documents ?? []} />
            </TabsContent>

            <TabsContent value="kanban" className="mt-0">
              <DataKanban data={tasks?.documents as any ?? []} onChange={onKanbanChange} />
            </TabsContent>

            <TabsContent value="calendar" className="mt-0 h-full pb-4">
              <DataCalendar data={tasks?.documents as any ?? []} />
            </TabsContent>
          </>
        )}
      </div>
